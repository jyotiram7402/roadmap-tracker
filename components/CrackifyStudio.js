    ...
    <div>
      <div className="flex items-center gap-3 mb-1">
        ...
        <div>
          ...
          <div className="text-[11px] text-zinc-400">A curated LeetCode starter set — tagged by type, difficulty &amp; pattern. Every one has a full brute → better → optimal walkthrough.</div>
        </div>
      </div>
      ...
      <div className="space-y-3 mt-4">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <select value={pattern} onChange={(e) => setPattern(e.target.value)}
            className="text-xs px-3 py-2 bg-[#141417] border border-white/[0.08] rounded-lg text-zinc-200 focus:outline-none focus:border-blue-500/60 max-w-[220px]">
          ...
          return (
            <button key={p.slug + i} onClick={() => setSelected(p)}
              ...
              <span className="flex-1 min-w-0">
                ...
                <span className="text-[11px] text-zinc-500 truncate block">{p.type} · {p.pattern}</span>
              </span>
              {solved(p)
