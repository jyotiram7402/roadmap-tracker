...
import StudyMaterial, { qaKey } from "@/components/StudyMaterial";
import StreakBadge from "@/components/StreakBadge";
import StageTOC from "@/components/StageTOC";
import FilterChips from "@/components/FilterChips";
import ExportMenu from "@/components/ExportMenu";
import { computeStreak, todayLocalDate, totalQaCount } from "@/lib/study-helpers";

const TOTAL = totalItems();
const TOTAL_QA = totalQaCount();
...
export default function Dashboard() {
  ...
  const [bookmarks, setBookmarks] = useState({});         // qaKey -> true
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const qm = {};
      (qa.data || []).forEach((r) => { qm[r.qa_key] = r.known; });
      setQaProgress(qm);

      const bmMap = {};
      (bm.data || []).forEach((r) => { bmMap[r.qa_key] = true; });
      setBookmarks(bmMap);
      setBookmarkCount((bm.data || []).length);
  ...
  const qaStats = useMemo(() => {
    const done = Object.values(qaProgress).filter(Boolean).length;
    return { done, total: TOTAL_QA, pct: TOTAL_QA ? Math.round((done / TOTAL_QA) * 100) : 0 };
  }, [qaProgress]);
  ...
  function stageQaStats(stageId) {
    const sections = studyMaterial[stageId] || [];
    let total = 0, done = 0;
  ...
  const filteredRoadmap = useMemo(() => {
    ...
    return ROADMAP
      .map((stage) => {
        const hasStudyMatch = (studyMaterial[stage.id] || []).some((s) =>
      ...
      .filter((stage) =>
        stage.sections.length > 0 || stage.title.toLowerCase().includes(q) || stage.hasStudyMatch
      );
  }, [search]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 text-slate-100">
      <header className="sticky top-0 z-20 bg-slate-900/95 backdrop-blur border-b border-slate-800 no-print">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <Link href="/flashcards" title="Flashcard mode" className="text-xs sm:text-sm px-2 sm:px-3 py-1.5 bg-pink-700/40 hover:bg-pink-700/60 border border-pink-800 rounded-lg">
              🎯 <span className="hidden sm:inline">Quiz</span>
            </Link>
            <Link href="/bookmarks" title="Bookmarks" className="text-xs sm:text-sm px-2 sm:px-3 py-1.5 bg-amber-700/30 hover:bg-amber-700/50 border border-amber-800 rounded-lg">
              ★ <span className="hidden sm:inline">{bookmarkCount}</span>
            </Link>
            <button onClick={signOut} className="text-xs sm:text-sm px-2 sm:px-3 py-1.5 bg-slate-800 hover:bg-slate-700 rounded-lg">
        ...
        <div className="max-w-5xl mx-auto px-4 pb-3 space-y-2">
          <input
            type="text"
            placeholder="Search topics, Q&A, code... (e.g. kafka, LRU cache, JWT)"
            value={search}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-xs text-slate-400 whitespace-nowrap">
                      {st.done}/{st.total}
                      {qst.total > 0 && (
                        <span className="text-pink-400 ml-2">· {qst.done}/{qst.total} Q&A</span>
                      )}
                    </span>
                  </div>
                </div>
                ...
                <div className="border-t border-slate-700">
                  {qst.total > 0 && (
                    <div className="flex items-center border-b border-slate-700 bg-slate-900/40">
                      <button
                        onClick={() => setStageTab((s) => ({ ...s, [stage.id]: "checklist" }))}
                      ...
                      <button
                        onClick={() => setStageTab((s) => ({ ...s, [stage.id]: "study" }))}
                        className={`flex-1 px-4 py-2.5 text-sm font-medium transition ${
                          tab === "study"
                      ...
                      <div className="px-3 flex-shrink-0">
                        <ExportMenu stageId={stage.id} stageTitle={stage.title} />
                      </div>
                    </div>
                  )}
                  ...
                  <div className="p-4 space-y-3">
                    {tab === "study" && qst.total > 0 ? (
                      <StudyMaterial
                        stageId={stage.id}
                        sections={studyMaterial[stage.id]}
                        search={search}
      ...
      <StageTOC stages={ROADMAP} stageStats={stageStats} onJump={jumpToStage} />
                ...
                <div className="flex items-start gap-2 group">
                  ...
                  <button
                    ...
                    className={`flex-shrink-0 text-xs px-1.5 py-0.5 rounded transition ${
                      hasNote ? "text-amber-400 bg-amber-400/10" : "text-slate-500 hover:text-amber-400 opacity-0 group-hover:opacity-100"
                    }`}
                  >📝</button>
                </div>
                {isNoteOpen && (
