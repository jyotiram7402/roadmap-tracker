...
export default function BookmarksPage() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-zinc-400">Loading bookmarks…</div>;
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        {bookmarks.length === 0 ? (
          ...
          Array.from(grouped.entries()).map(([groupKey, items]) => {
            ...
            return (
              <div key={groupKey}>
                <h2 className="font-bold text-cyan-400 text-sm sm:text-base mb-2 sticky top-12 bg-[#141417]/90 py-2 -mx-4 px-4 backdrop-blur">
                  {meta.icon} {meta.name} <span className="text-zinc-500 font-normal">· {items.length}</span>
                </h2>
                <div className="space-y-2">
                  {items.map((b) => {
                    ...
                    return (
                      <div key={b.qa_key} className="bg-[#18181b] border border-white/[0.06] rounded-lg overflow-hidden">
                        <div className="flex items-stretch">
                          <button onClick={() => setOpenKey(isOpen ? null : b.qa_key)} className="flex-1 text-left px-3 py-2.5 hover:bg-white/[0.06] flex items-start gap-2 min-w-0">
                            <span className="text-blue-400 font-mono text-xs mt-0.5">Q{b.q_num}</span>
                            <span className="flex-1 text-sm text-zinc-100 break-words min-w-0">{b.q_text}</span>
                            <span className="text-zinc-500 flex-shrink-0">{isOpen ? "−" : "+"}</span>
                          </button>
                          <button onClick={() => remove(b.qa_key)} title="Remove bookmark" className="w-9 flex items-center justify-center border-l border-white/[0.06] text-amber-400 hover:text-rose-400">★</button>
                        </div>
                        {isOpen && (
                          <div className="px-3 pb-3 space-y-2 border-t border-white/[0.06]/50 pt-2">
                            {q ? q.blocks.map((blk, i) => {
                              if (blk.kind === "text") return <p key={i} className="text-sm text-zinc-300 leading-relaxed">{blk.text}</p>;
                              ...
                              if (blk.kind === "diagram") return <pre key={i} className="bg-[#0c0d12] border border-blue-900/50 rounded p-3 overflow-x-auto text-xs"><code className="text-blue-300 font-mono whitespace-pre">{blk.lines.join("\n")}</code></pre>;
                              return null;
                            }) : <p className="text-xs text-zinc-500">Answer unavailable (content updated). Open the track to view.</p>}
                          </div>
                        )}
                      </div>
                    );
