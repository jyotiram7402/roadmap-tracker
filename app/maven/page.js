        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 pb-3 flex gap-2">
          {[["notes", "📖 Notes"], ["interview", "❓ Interview Questions"]].map(([v, label]) => (
            <button key={v} onClick={() => setTab(v)}
              className={`text-sm px-3 py-1.5 rounded-lg font-medium whitespace-nowrap border transition ${tab === v ? "bg-blue-600 border-blue-500 text-white" : "bg-[#18181b] border-white/[0.06] text-zinc-400 hover:border-white/[0.14] hover:text-zinc-200"}`}>
              {label}
            </button>
          ))}
        </div>
...
function NoteBlock({ block }) {
  ...
  // text -> bullet point
  return (
    <div className="my-1.5 flex gap-2 text-sm text-zinc-200 leading-relaxed">
...
function NotesTab() {
  ...
  if (topics === undefined) return <div className="text-center py-16 text-zinc-500 animate-pulse">Loading notes…</div>;

  if (topics.length === 0) {
    return (
      <div className="text-center py-16 text-zinc-400">
        <div className="text-4xl mb-3">📖</div>
        <p className="font-semibold text-zinc-300">Maven notes — ready for your content</p>
        <p className="text-sm mt-2 max-w-md mx-auto">
          I couldn&apos;t read the PDFs here (no PDF renderer, and the text is stored as glyph codes / drawings).
          Paste the notes as text, or re-send them as <span className="text-zinc-200">images (PNG/JPG)</span> — I can read images directly —
  ...
  return (
    <div className="space-y-6">
      {/* table of contents */}
      <div className="rounded-xl border border-white/[0.06] bg-[#1c1c20]/40 p-3">
        <div className="text-[11px] uppercase tracking-wide text-zinc-500 mb-1.5">Topics</div>
        <div className="flex flex-wrap gap-2">
          {topics.map((t) => (
            <a key={t.slug} href={`#${t.slug}`} className="text-xs px-2 py-1 rounded-lg bg-[#141417] border border-white/[0.06] text-zinc-300 hover:border-white/[0.14]">{t.title}</a>
          ))}
        </div>
      </div>
      {topics.map((t) => (
        <section key={t.slug} id={t.slug} className="scroll-mt-24 rounded-xl border border-white/[0.06] bg-[#1c1c20]/40 p-4 sm:p-5">
...
function InterviewTab() {
  ...
  if (questions === undefined) return <div className="text-center py-16 text-zinc-500 animate-pulse">Loading questions…</div>;
  if (questions.length === 0) {
    return (
      <div className="text-center py-16 text-zinc-400">
        <p className="font-semibold text-zinc-300">Maven interview questions are being added.</p>
