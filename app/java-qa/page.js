...
import { JAVA_QA_CATEGORIES, loadJavaQa, googleLink } from "@/data/java-qa";

export default function JavaQaPage() {
  const [cat, setCat] = useState("oops");
  ...
  useEffect(() => {
    ...
    setSelected(null);
    setQ("");
    loadJavaQa(cat).then((list) => { if (ok) setQuestions(list); });
    return () => { ok = false; };
  }, [cat]);

  const meta = JAVA_QA_CATEGORIES.find((c) => c.id === cat);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 text-slate-100 flex flex-col">
      <header className="sticky top-0 z-20 bg-slate-900/95 backdrop-blur border-b border-slate-800">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3 flex items-center gap-3">
          ...
          <h1 className="text-base sm:text-lg font-bold flex-1 min-w-0 truncate">📘 Java Interview Q&amp;A</h1>
          <LiveClock className="hidden sm:flex" />
        </div>
        {/* category tabs */}
        <div className="max-w-5xl mx-auto px-4 sm:px-6 pb-3 flex gap-2 overflow-x-auto">
          {JAVA_QA_CATEGORIES.map((c) => (
      ...
      <main className="max-w-5xl mx-auto w-full px-4 sm:px-6 py-5 flex-1">
        {selected ? (
          <QuestionView cat={cat} item={selected} onBack={() => setSelected(null)} />
        ) : (
          <>
      ...
      <footer className="border-t border-slate-800 py-6 text-center text-xs text-slate-500">
        ...
        Java Interview Q&amp;A · answers written for interviews, with examples & code
      </footer>
    </div>
  );
}
...
function QuestionView({ cat, item, onBack }) {
  return (
    <div>
      ...
      <MySolution slug={`java-qa:${cat}:${item.slug}`} />
    </div>
  );
}
