...
import { TRACKS, DEFAULT_TRACK, loadTrackData, qaKey } from "@/data/tracks";
...
export default function FlashcardsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setUser(user);
      try {
        const saved = localStorage.getItem("roadmap.activeTrack");
      ...
      const m = {}; (data || []).forEach((r) => { m[r.qa_key] = r.known; });
  ...
  const next = useCallback(() => { setFlipped(false); setPos((p) => (p + 1) % Math.max(1, order.length)); }, [order.length]);
  const prev = useCallback(() => { setFlipped(false); setPos((p) => (p - 1 + order.length) % Math.max(1, order.length)); }, [order.length]);
  const reshuffle = useCallback(() => { setOrder(shuffle(pool.length)); setPos(0); setFlipped(false); }, [pool.length]);
        ...
        {dataLoading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-2 mt-4">
              <button onClick={() => markKnown(false)} className="px-3 py-2.5 bg-rose-900/40 hover:bg-rose-900/60 border border-rose-800 rounded-lg text-sm text-rose-200">✗ Review (U)</button>
